#!/usr/bin/env python3
"""Compute Cohen's kappa for two pilot annotation CSV exports."""
...
def render_markdown(results: Dict[str, CohenKappaResult], *, aligned) -> List[str]:
    lines = [
        "# Cohen's kappa — pilot 001",
        "",
    ...
    for column in ANNOTATION_COLUMNS:
        ...
        for label, stats in sorted(result.per_label.items()):
            extra = ""
            if "binary_kappa" in stats:
                extra = f" (binary κ={_format_float(float(stats['binary_kappa']))})"
            lines.append(
                f"| `{label}` | {int(stats['support_a'])} | {int(stats['support_b'])} | "
                f"{int(stats['agree_both'])} | {_format_float(float(stats['agree_rate']))}{extra} |"
            )
    return lines
...
def run(annotator_a: Path, annotator_b: Path, output_dir: Path) -> Dict[str, CohenKappaResult]:
    ...
    (output_dir / "cohen_kappa.json").write_text(
        json.dumps(json_safe(payload), indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    write_markdown(output_dir / "cohen_kappa.md", render_markdown(results, aligned=aligned))
    return results


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compute Cohen's kappa for pilot annotations.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print(f"Wrote Cohen's kappa report to {args.output_dir}")
    return 0
